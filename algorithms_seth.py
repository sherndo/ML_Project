...
def trainXGBoost(data, classes, k_fold, scoring):
    ...
    # gbc = XGBClassifier(n_jobs=8, random_state=7, max_depth=3, booster='dart', learning_rate=0.1, colsample_bytree=0.3, gamma=0.4, min_child_weight=3)
...
# {'booster': 'gbtree', 'colsample_bytree': 0.3, 'gamma': 0.2, 'learning_rate': 0.1, 'max_depth': 3, 'min_child_weight': 1, 'random_state': 7, 'subsample': 0.7}
    ...
    # gbc_mean = numpy.mean(gbc_scores['estimator'], axis=0)
    # gbc_std = numpy.std(gbc_scores['estimator'], axis=0)

    for i in range(len(gbc_scores['estimator'])):
        # x = gbc_scores['estimator'][i].feature_importances_()
        plot_importance(gbc_scores['estimator'][i], importance_type='cover')
        plt.savefig('/home/sherndo/Documents/Classes/MachineLearning/ML_Project/xg_'+str(i)+'_features.png')
